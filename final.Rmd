...
#   subsets <- regsubsets(formula(model),
#                         model.frame(model),nbest=nbest,nvmax=nvmax,force.in=force.in,...)
#   subsets <- with(summary(subsets),
...
# InterTab = round(best(lm(Y~.+.*. , data), nbest = 2,nvmax=29, force.in = 2:10), 4) %>% 
#   as.data.frame()
...
Bonferroni critical value = 3.68 ( $\alpha = 0.1$ )  
Leverage critical value = 0.5 (?? 2p/n = 0.054)
...
DFFITS critical value = 0.33 ( $2\sqrt{p/n}$ )  
